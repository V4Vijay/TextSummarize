...
def clean_transcript(transcript):
  ...
  for line in transcript:
    ...
    #cleaned_transcript.append({"text": text, "speaker": line["speaker"]}) Removed part: Error extracting transcript: 'speaker'
...
# Load Transcript:
# Replace with your actual path to the saved transcript file
...
# Optional: Remove stop words and punctuation
from nltk.corpus import stopwords
stop_words = stopwords.words('english')

cleaned_sentences = [
    word.lower() for word in sentence.split() if word.lower() not in stop_words and word.isalnum()
]

# Convert list of words to string for summarizing
processed_transcript = " ".join(cleaned_sentences)

# Summarize Transcript

# Use TextRank algorithm (built-in nltk)
from nltk.summarization import TextRank

tr = TextRank()
summary = tr.summarize(processed_transcript, sentences_count=5)  # Adjust as needed

# Print or save the summary
print(summary)
# Or write to a file: with open("summary.txt", "w") as f: f.write(summary)
